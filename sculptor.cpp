...
void Sculptor::WriteOFF(string arquivo){
    ...
    fout.open("C:/Users/guga_/Documents/Downloads/"+arquivo+".off");
...
void Sculptor::WriteVECT(string arquivo){
    ...
    fout2.open("C:/Users/guga_/Documents/Downloads/"+arquivo+".vect");
    ...
    for(int i=0; i<nLinhas; i++)
        for(int j=0; j<nColunas; j++)
            for(int k=0; k<nPlanos; k++)
                if(v[i][j][k].temBloco)
                    fout2<<v[i][j][k].vermelho<<" "<<v[i][j][k].verde<<" "<<v[i][j][k].azul<<" "<<v[i][j][k].transparencia<<endl;
